...
**K-均值**是一个==迭代算法==，它会重复做两件事，第一个是==簇分配==，第二个是==移动聚类中心==。

**K-Means**用图像说明再合适不过了，假设有一个无标签的数据集，如下图，现在我们想将其分为两个簇：



![](../images/ff1db77ec2e83b592bbe1c4153586120.png)

**K-Means**算法过程如下：

1. ==随机初始化==：随机生成2个点，这两点叫做“聚类中心”。

![](../images/acdb3ac44f1fe61ff3b5a77d5a4895a1.png)

2. ==簇分配==：遍历每个样本，根据每个样本点距离哪个“聚类中心”更近，将样本点分配给聚类中心。

![](../images/fe6dd7acf1a1eddcd09da362ecdf976f.png)

3. ==移动聚类中心==：将“聚类中心”移动到同类样本的均值处。

![](../images/ff1db77ec2e83b592bbe1c4153586231.png)

4. ==不断迭代==：重复 步骤2-步骤3 ，直到聚类中心不再改变，点的颜色也不会改变了。

![](../images/ff1db77ec2e83b592bbe1c4153545213.png)

此时，可以说**K均值**已经聚合了，该算法在找出数据中的簇方面做的相当好。

下面，我们用更规范的格式，写出**K均值**算法：

1. 接收两个输入值：

![](../images/ff1db77ec2e8f4592bbe1r12543215123.png)



2. 迭代过程：

   ![](../images/ff1db77ec2e8f4592bbe1r12546235621.png)
...
​    ${\mu}_{c^{(i)}}$表示第$i$个样本点所属的簇的聚类中心

即优化目标为：样本点到样本点所属的聚类中心的距离。
我们的的优化目标便是找出使得代价函数最小的 $c^{(1)}$,$c^{(2)}$,...,$c^{(m)}$和$μ_1$ , $μ_2$ , ... , $μ_k$：
![](../images/8605f0826623078a156d30a7782dfc3c.png)

回顾刚才给出的**K-均值**迭代算法，我们知道它实质是在做如下的事情：

​    第一个循环是用于减小$c^{(i)}$引起的代价，因为第一个循环中聚类中心是固定的，所以只能通过分配最近的样本点，才能达到样本点到所属聚类中心距离之和最短。

​    第二个循环则是用于减小${{\mu }_{i}}$引起的代价，因为第二个循环中样本点是不分配的，所以只能通过移动聚类中心至样本簇的均值处，才能到达距离之和最短。
...
例如，对于下面左图所示的数据集，理想的聚类结果应该如右图所示那样：

![](../images/d4d2c3edbdd8915f4e9d254d2a47d9c6.png)

但如果随机初始化的结果不太好，实际的聚类结果可能会达到局部最优，如下面左图或右图那样：

![](../images/d4d2c3edbdd8915f4e9d254d2a47d9c8.png)

为了解决这个问题，我们通常需要多次运行**K-均值**算法，每一次都重新进行随机初始化，最后再比较多次运行**K-均值**的结果，选择==代价函数最小的结果==。这种方法在$K$较小的时候（2--10）效果很明显。当$K$较大时，很可能第一次随机初始化就能得到相当好的结果，多次运行可能会给你稍好一点的结果，但不会好太多。
...
我们可能会得到一条类似于==上面左图==这样的曲线。像一个人的肘部。这就是“肘部法则”所做的，让我们来看这样一个图，看起来就好像有一个很清楚的肘在那儿。好像人的手臂，如果你伸出你的胳膊，那么这就是你的肩关节、肘关节、手。这就是“肘部法则”。你会发现这种模式，它的畸变值会迅速下降，从1到2，从2到3之后，你会在3的时候达到一个肘点。在此之后，畸变值就下降的非常慢，看起来就像使用3个聚类来进行聚类是正确的，这是因为那个点是曲线的肘点，畸变值下降得很快，$K=3$之后就下降得很慢，那么我们就选$K=3$。当你应用“肘部法则”的时候，如果你得到了一个像上面这样的图，那么这将是一种用来选择聚类个数的合理方法。

但我们也经常会得到==上面右图==那样的曲线，它并没有一个清晰的拐点，看上去畸变值是连续下降的，这种情况下用“肘部法则”来选择聚类数目是很困难的。

对于“肘部法则”，它是一个值得尝试的方法，但是你并不能期望它能够解决任何问题。

推荐方法二：基于**K均值**聚类的用途

另外一个选择K值的思路，通常人们使用**K均值**聚类是为了某个用途，然后选择能最好服务于该目标的聚类数。

例如，你想用**K均值**聚类进行市场分割，正如我们的 T-恤制造例子中，我们要将用户按照身材聚类，准备分成3个尺寸:$S,M,L$，也可以分成5个尺寸$XS,S,M,L,XL$，这样的选择是建立在回答“聚类后我们制造的T-恤是否能较好地适合我们的客户”这个问题的基础上作出的。

或者你想用**K均值**来更好地组织计算机集群，或者为了别的用途。
...
对于降维后得到的上图的理解，可以认为$z1$轴代表着全国GDP，$z2$轴代表人民幸福指数(或人均GDP)。
...
​        我们需要计算出所有特征的均值${\mu}_{j}$，然后令 $x_j= x_j-μ_j$。归一化以后，每个特征的均值将变为0。

​        如果特征是在不同的数量级上，我们还需要特征缩放，计算每个特征的标准差${\sigma}^{2}$，然后令 ${x}_{j}=\frac{{x}_{j}-{\mu}_{j}}{{\sigma}^{2}}$。（其实用波动范围即最大值减最小值代替${\sigma}^{2}$效果是一样的。）

第二步是计算**协方差矩阵**（**covariance matrix**）$Σ$：
$\sum=\dfrac {1}{m}\sum^{n}_{i=1}\left( x^{(i)}\right) \left( x^{(i)}\right) ^{T}$       注：${x}^{(i)}$是一个 $n{\times}1$维的向量，所以最终 $\Sigma$ 是一个 $n{\times}n$ 维的矩阵

第三步是计算协方差矩阵$Σ$的**特征向量**（**eigenvectors**）:

在**Octave**中用命令：**[U, S, V] = svd(sigma)**​

svd代表奇异值分解，**Singular value decomposition**，这是一个更加高级的分解算法，也是一个很高级的线性代数的应用，但你只要知道sigma代表着上面的矩阵$\Sigma$。

有几个方法可以在高维向量中进行计算，如果你很擅长线性代数，**Octave**中的 **eig** 函数用来计算和 **svd** 相同的事，已经证明它俩会有相同的结果，但 **svd** 在数值上更稳定。当你用 **eig** 函数求一个协方差矩阵，**svd** 会给你一个相同的结果，因为协方差矩阵总是满足一个数学上的性质叫做“正定矩阵”。

![](../images/0918b38594709705723ed34bb74928ba.png)

**svd** 最终的输出是三个矩阵$U$, $S$, $V$，其中 $U$也是一个 $n \times n$维的矩阵，包含列向量 $U^{(1)},U^{(2)} ... U^{(n)}$。它是一个具有与数据之间最小投射误差的方向向量构成的矩阵。

如果我们希望将数据从$n$维降至$k$维，我们只需要从$U$中选取前$k$个向量，获得一个$n×k$维度的矩阵，我们用$U_{reduce}$表示，然后通过如下计算获得要求的新特征向量$z^{(i)}$:
...
1. 进行归一化处理，保证每个特征的均值为 0。必要时还需进行特征缩放。
2. 求载体矩阵，即协方差矩阵 $\Sigma$
3. 用 **svd** 函数求出矩阵 $U$, $S$, $V$，从矩阵 $U$ 中取出前 **K** 列（K为要降到的维度），记为 $U_{reduce}$
4. 所有数据最终在降维后空间上的投影为：$Z=U_{reduce}{\times}X$
...
投影误差平方的平均值为：$\frac{1}{m}\sum^{m}_{i=1}\|x^{(i)}-x^{(i)}_{approx}\|^2$

训练集的总方差为：$\dfrac {1}{m}\sum^{m}_{i=1}\left\| x^{\left( i\right) }\right\| ^{2}$        （==即：每个样本到坐标原点的距离平均==）
...
$$\dfrac {\dfrac {1}{m}\sum^{m}_{i=1}\left\| x^{\left( i\right) }-x^{\left( i\right) }_{approx}\right\| ^{2}}{\dfrac {1}{m}\sum^{m}_{i=1}\left\| x^{(i)}\right\| ^{2}}=1-\dfrac {\Sigma^{k}_{i=1}S_{ii}}{\Sigma^{n}_{i=1}S_{ii}}\leq 1\%$$
...
假使我们正在针对一张 100×100像素的图片进行某个计算机视觉的机器学习，即总共有10000 个特征。
      ...
      1. 从原始训练样本$(x^{(i)},y^{(i)})$中提取所有的特征$x^{(i)}$，然后运用**PCA**降维得到低维特征$z^{(i)}$，即新的训练样本$(z^{(i)},y^{(i)})$。
      2. 用$(z^{(i)},y^{(i)})$运行学习算法，如逻辑回归，神经网络，SVM等等
      3. 在预测时，采用之前学习而来的$U_{reduce}$将输入的特征$x$转换成特征向量$z$，然后再进行预测

注：PCA所做的是定义了一个$x$到$z$的映射，这个映射只能通过在训练集上运行PCA来定义。具体而言，PCA学习的这个映射所做的就是计算一系列参数，进行特征缩放、均值归一化、矩阵$U_{reduced}$，所有这些东西应该只在训练集上拟合，而不是验证集或测试集。如果我们有交叉验证集和测试集，也采用PCA在训练集上学习来的$U_{reduce}$进行转换。
